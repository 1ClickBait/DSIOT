...
*/
bool sendLoRaFrame(float si, float pga)
{
  ...
  // Convert SI and PGA to uint16_t (scale *1000 for milliprecision)
  uint16_t scaledSI  = (uint16_t)(si * 1000);   // Example: 1.234 m/s -> 1234
  uint16_t scaledPGA = (uint16_t)(pga * 1000);  // Example: 0.987 m/s^2 -> 987

  // Add to LoRa buffer with identifiers
  m_lora_app_data_buffer[buffSize++] = 0x10; // ID for SI
  m_lora_app_data_buffer[buffSize++] = scaledSI >> 8;
  m_lora_app_data_buffer[buffSize++] = scaledSI & 0xFF;

  m_lora_app_data_buffer[buffSize++] = 0x11; // ID for PGA
  m_lora_app_data_buffer[buffSize++] = scaledPGA >> 8;
  m_lora_app_data_buffer[buffSize++] = scaledPGA & 0xFF;
